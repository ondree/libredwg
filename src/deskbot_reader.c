...
static char *
entityTextValue (Dwg_Data *data, BITCODE_TV value)
{
...
int
countChar (const char *str, char symbol)
{
  int count = 0;
  for (int i = 0; str[i]; i++)
    {
...
static void
insertData (DeskbotData *deskbotData, Attribute *attribute,
...
static bool
layerNamesWithPrefix (Dwg_Entity_POLYLINE_2D entity, const char *seatLayer,
...
static BITCODE_RD
countRotation (Polygon polygon, BITCODE_3BD insertPoint)
{
            ...
            PolygonList *polygonList, DeskbotInsert insert, Dwg_Handle handle)
{
  ...
  polygon.rotation = countRotation (polygon, insert.insertPoint);
  insertPolygonList (polygonList, polygon);
}

static void
loadAttribute (Dwg_Data *data, Dwg_Entity_ATTRIB *entity, Attribute *attribute)
{
...
static void
loadAttributes (Dwg_Data *data, Dwg_Entity_INSERT *insertEntity,
