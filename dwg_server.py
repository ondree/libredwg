...
async def parse_image(request: Request):
    ...
    execute = 'dwg2SVG ' + original_file_path + ' >' + target_file_path
...
async def parse_file(request: Request):
    ...
    if not file:
        return json({'message': 'No file selected!', 'success': False})
    if not file.name.split('.')[-1] == 'dwg':
        return json({'message': 'Selected file is not an dwg!', 'success': False})

    await save_file(file)

    name_of_file = file.name.split('.')
    name_of_file.pop()

    os.system('./dwgread_test ' + os.path.join(os.getcwd(), file.name))
