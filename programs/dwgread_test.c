...
void printDetails (Dwg_Data *pStruct);
void printDwgDetails (Dwg_Data *pStruct);
void printDwgBlocks (Dwg_Data *pStruct);
void printObjects (Dwg_Object_Object *objects);
void transform_OCS2 (BITCODE_3DPOINT *out, BITCODE_3DPOINT pt, BITCODE_BE ext);
...
main (int argc, char *argv[])
{
  ...
  static struct option long_options[]
      = { { "verbose", 1, &opts, 1 }, // optional
          ...
          { "rooms", 1, 0, 'R' },     { "seats", 1, 0, 'S' },
          { "labels", 1, 0, 'L' },    { "help", 0, 0, 0 },
  ...
  while
#ifdef HAVE_GETOPT_LONG
      ((c = getopt_long (argc, argv, ":v::O:o:h:R:S:L", long_options,
                         &option_index))
       != -1)
#else
...
#  endif
          ...
          if (!strcmp (long_options[option_index].name, "force-free"))
            force_free = 1;
          if (!strcmp (long_options[option_index].name, "seats"))
            seatLayer = optarg;
          break;
#else
        case 'i':
